...
describe('Snapshot Loading', () => {
    ...
    test('Loading ordinal', () => {
        const type = 'Actor'
        const ordinal = 2
        const hobj = TypeIterator.createGenericObject(engine, type, ordinal);
        console.log('loading ', type, 'ordinal', ordinal);
        const obj = RecordStringifier.hollowObjectToValue(hobj)
        expect(obj.actorId).toBe(20003)
        expect(obj.actorName.value).toBe('Actor 20003')
    })
