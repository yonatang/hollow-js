...
describe('BitSet', () => {
  ...
  test('should find previous set bit', () => {
    const bitSet = new BitSet(128);
    ...
    expect(bitSet.previousSetBit(100)).toBe(100);
    expect(bitSet.previousSetBit(99)).toBe(63);
    expect(bitSet.previousSetBit(62)).toBe(32);
    expect(bitSet.previousSetBit(31)).toBe(29);
    expect(bitSet.previousSetBit(28)).toBe(10);
    expect(bitSet.previousSetBit(9)).toBe(0);
    expect(bitSet.previousSetBit(0)).toBe(0);
    expect(bitSet.previousSetBit(-1)).toBe(-1);
  });

  test('should find previous set bit across word boundary', () => {
    const bitSet = new BitSet(128);

    bitSet.set(63);
    bitSet.set(64);

    expect(bitSet.previousSetBit(64)).toBe(64);
    expect(bitSet.previousSetBit(63)).toBe(63);
    expect(bitSet.previousSetBit(65)).toBe(64);
  });

  test('should return -1 from previousSetBit on empty BitSet', () => {
    const bitSet = new BitSet(64);
    expect(bitSet.previousSetBit(63)).toBe(-1);
    expect(bitSet.previousSetBit(0)).toBe(-1);
  });
