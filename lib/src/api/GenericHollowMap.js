...
export class GenericHollowMap {
  ...
  /**
   * Find the key/value ordinals for a logical index
  ...
  findEntryByIndex(index) {
    const entries = this.typeState.getMapEntries(this.ordinal);
    if (index >= entries.length) {
      throw new Error(`Index ${index} out of bounds for map of size ${this.size()}`);
    }
