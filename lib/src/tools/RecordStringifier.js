...
export class RecordStringifier {
  ...
  static objectToValue(obj) {
    ...
    // Unwrap single-STRING-field OBJECT types (e.g. the hollow String wrapper type)
    if (schemaType === SchemaType.OBJECT && schema.numFields() === 1 &&
        schema.getFieldType(0) === FieldType.STRING) {
      return obj.getString(schema.getFieldName(0));
    }
