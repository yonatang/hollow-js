...
export class BlobInput {
  ...
  /**
   * Read `count` big-endian 64-bit words and return them as a little-endian BigUint64Array.
   * Performs the byte-swap in a plain Uint8 loop to avoid BigInt in the hot path.
   * @param {number} count - Number of 64-bit words to read
   * @returns {BigUint64Array}
   */
  readBigUint64ArrayBE(count) {
    const numBytes = count * 8;
    if (this.position + numBytes > this.buffer.byteLength) {
      throw new Error('Unexpected end of input');
    }
    const src = new Uint8Array(this.buffer, this.position, numBytes);
    const dst = new ArrayBuffer(numBytes);
